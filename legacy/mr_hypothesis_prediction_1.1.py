...
logging.info(f"Number of non_predictions: {len(non_predictions)}")
...
for i, play_title in enumerate(all_chunk_play_titles):
    ...
    play_predictions[play_title].append(non_predictions[i])
...
non_results = [{"play_title": title.replace("non_", ""), "prediction": prediction} for title, prediction in final_play_predictions.items()]
