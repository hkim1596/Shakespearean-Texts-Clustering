...
good_texts = [play_text for tcp in tqdm(good_texts_df["TCP"]) 
              for play_text in parse_file(os.path.join(data_dir, tcp + ".xml"))]
print(f"Processed {len(good_texts)} plays for Good Texts.")
#print(good_texts[:10])

bad_texts = [play_text for tcp in tqdm(bad_texts_df["TCP"]) 
             for play_text in parse_file(os.path.join(data_dir, tcp + ".xml"))]
print(f"Processed {len(bad_texts)} plays for Bad Texts.")
#print(bad_texts[:10])

non_texts = [play_text for tcp in tqdm(non_texts_df["TCP"]) 
             for play_text in parse_file(os.path.join(data_dir, tcp + ".xml"))]
print(f"Processed {len(non_texts)} plays for Non Texts.")
#print(non_texts[:10])
