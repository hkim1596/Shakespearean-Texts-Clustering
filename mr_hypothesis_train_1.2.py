...
def log_results_to_file(message, filename="training_details.txt"):
...
def log_time(start_time, task_name="Task"):
    ...
    log_results_to_file(f"{task_name} completed in {elapsed_time:.2f} seconds.")
...
def parse_file(file_path):
    ...
    log_results_to_file(f"Processing file: {os.path.basename(file_path)}")

    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()
        if not content:
            print(f"File {file_path} is empty.")
            log_results_to_file(f"File {file_path} is empty.")
            return []
        ...
        log_results_to_file(f"Extracted titles: {titles}")
...
log_results_to_file(f"Total chunks to process: {total_chunks}")
...
for key, value in train_results.items():
    log_results_to_file(f"{key}: {value}")
...
log_results_to_file(f"Accuracy: {accuracy}")
print(classification_report(test_labels, predictions))
log_results_to_file(classification_report(test_labels, predictions))
